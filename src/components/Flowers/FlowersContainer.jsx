...
const FlowersContainer = connect(mapStateToProps,
    //  mapDispatchToProps
    { chooseCategory: chooseCategoryActionCreator, addToOrder: addToOrderActionCreator, showItem: onShowItemActionCreator }
)(Flowers);

export default FlowersContainer;

// chooseCategory - метод для вибору категорії dispatch(chooseCategoryActionCreator(el.key); + в метод передається (el.key) = (categories)
// addToOrder - метод для додавання товару в корзину покупок dispatch(addToOrderActionCreator(item); + в метод передається item
// showItem - // метод dispatch(onShowItemActionCreator(props.item)) показати товар у модальному вікні; + в метод передається item */}